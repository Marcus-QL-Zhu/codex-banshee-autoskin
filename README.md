...
用 CDP 注入给 Codex 换肤这个思路的初版（v1）出自我之手。后来很高兴看到这个玩法在社区里传开，出现了各种分支和衍生版本，加了新平台、新主题——这正是开源该有的样子。

这份 2.0 是我对这个想法的全面重写：v1 回答的是"能不能给 Codex 换肤"，2.0 回答的是"怎么让**任何人发一张图**就得到一套自己的皮肤"。
...
Codex Dream Skin 2.0 — a manifest-driven skin engine for the Windows Codex desktop app, a full rewrite by the author of the original v1 CDP-injection skin (glad to see the idea spread through community forks and derivatives — that's open source working as intended).
