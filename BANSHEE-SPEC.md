...
- The side cavities use one `1600`-unit continuous gradient band with a shallow head/tail falloff. Its cycle is `2s`, while its animation start time remains attached to the shared document-timeline epoch. It descends in screen space from top to bottom and is revealed only where the accepted cavity clip is present. At the first frame the band is wholly above the cavity and its lower edge touches the cavity top at `y=65`; at the final frame the band is wholly below the cavity and its upper edge touches the cavity bottom at `y=848`. With the `909`-unit SVG view box this is encoded as `translateY(-168.866%)` to `translateY(93.289%)`. The gradient itself supplies the zero-luminance entry and exit edges, so no separate opacity fade may shift these physical boundary phases.
