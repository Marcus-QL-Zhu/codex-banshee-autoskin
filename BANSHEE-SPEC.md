...
Header relocation follows the same fail-closed two-stage policy as other required surfaces. The fallback candidate set is limited to direct `header` children of the verified main surface and must independently match native controls, a title/control cluster, rendered toolbar geometry, and toolbar semantics. The complete native header remains one unit so the title menu and the summary, bottom-panel, and sidebar toggles retain their original order and hit targets inside the safe band.
...
The renderer already uniquely classifies and parity-checks native composer controls. Two persistent native Fast representations are supported without label-text inference. A dedicated Fast button is read from its exact accessible-name allowlist plus `aria-pressed`. Current Codex builds instead expose Fast inside the native model/reasoning trigger: that button must uniquely carry both `data-codex-intelligence-trigger="true"` and `data-composer-navigation-target="reasoning"`; enabled state additionally requires the existing inline lightning SVG to retain either the legacy semantic `ModelPickerTriggerInlineFastIcon` class fragment or the newer `ModelPickerTriggerInlineModeIcon` class fragment, together with the lightning-specific `0 0 24 24` view box and current-color path. The geometry check distinguishes the newer Fast lightning from other native mode indicators that reuse the same class with a different view box. While the model popup is open, its unique native `menuitemcheckbox` is a higher-priority live state source only when `data-fast-mode-enabled` and `aria-checked` agree, it remains inside the open native Radix model menu, and it retains an SVG indicator. This avoids depending on the popup icon's CSS-module class, which Codex 26.901 replaced with a generic icon class. A verified native click predicts the next boolean state and switches the palette marker in capture phase before Codex performs its potentially expensive subtree replacement. Attribute and subtree mutations schedule the normal reconciler, which confirms the resulting native state after the interaction settles. This keeps mouse and keyboard activation visually immediate without letting transient intermediate replacement nodes overwrite the predicted state; the native state remains authoritative at reconciliation. After the existing verified/parity gate, the renderer sets `data-dream-fast="on"` on the pack root only for a verified on state, and removes the marker for off, unavailable, ambiguous, or failed-parity states. Palette variables are overridden beneath that single marker. The switch must not restart the shared animation epoch, alter phase, rebuild SVG chrome, or replace/reshape the native lightning icon. CSS may change its inherited `currentColor` only beneath the verified Fast-on root marker.
...
- Composer relocation is bounded to rendered editor ancestors inside the verified main surface. It must independently match editor containment, main containment, visibility, composer-root semantics, native controls, plausible composer geometry, and bottom anchoring; a body/footer lookalike must lose by the configured confidence margin.
